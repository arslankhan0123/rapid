 ...
 */
class ManualSalesRepository extends BaseRepository
{
    /**
     * @var array
     */
    ...
    {
        if (!empty($customerId)) {
            return ManualSale::selectRaw('count(case when payment_status = 0 then 1 end) as drafted')
    ...
    public function getProjectOnly()
    {
    ...
    /**
     * @param  array  $input
     * @param  null|ManualSale|CreditNote|Estimate  $owner
     * @return bool
     */
    ...
    /**
     * @param  array  $input
     * @param  ManualSale|CreditNote|Estimate  $owner
     * @return bool
     */
   ...
   // public function prepareInvoiceData($input)
    // {
    //     $invoiceFields = (new ManualSale())->getFillable();
    //     $items = [];
    ...
    public function updateInvoice($input, $id)
    {
        ...
        $oldUserIds = ManualSale::whereId($invoice->id)->get()->pluck('sales_agent_id')->toArray();
        ...
        $contactIds = ManualSale::whereId($id)->pluck('customer_id')->toArray();
    ...
    public function deleteInvoice($invoice)
    {
        ...
        activity()->performedOn($invoice)->causedBy(getLoggedInUser())
            ->useLog('Invoice deleted.')->log($invoice->invoice_number . ' Invoice deleted.');
    ...
    {
        return ManualSale::whereId($id)->update(['payment_status' => $paymentStatus]);
    }
    ...
    public function getNotesData($invoice)
    {
        return Note::with('user.media')->where('owner_id', '=', $invoice->id)
            ->where('owner_type', '=', ManualSale::class)->orderByDesc('created_at')->get();
    }
    public function getInvoice($invoiceNumber)
    {
        return ManualSale::with([
            'customer',
            'salesItems',
    ...
    public function approve($manualSale)
    {
        $manualSale->update([
            'status' => 1 // status = 1
        ]);
