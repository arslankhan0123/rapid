...
use App\Models\PurchaseInvoiceDoc;
...
use App\Models\PurchaseInvoiceTerm;
...
class PurchaseInvoiceRepository extends BaseRepository
{
    ...
    public function storeTerms($id, $input)
    {
        ...
        PurchaseInvoiceTerm::where('estimate_id', $id)->delete();
        ...
        if (!empty($estimateTerms)) {
            PurchaseInvoiceTerm::insert($estimateTerms);
        }
    }
    ...
    {
        $invoiceFields = (new PurchaseInvoice())->getFillable();
        $items = [];
    ...
    public function updateInvoice($input, $id)
    {
        ...
        $oldUserIds = PurchaseInvoice::whereId($invoice->id)->get()->pluck('sales_agent_id')->toArray();


        $userId = implode(' ', $oldUserIds);
        $contactIds = PurchaseInvoice::whereId($id)->pluck('customer_id')->toArray();
    ...
    public function deleteInvoice($invoice)
    {
        ...
        activity()->performedOn($invoice)->causedBy(getLoggedInUser())
            ->useLog('Purhcase Invoice  deleted.')->log($invoice->title . 'Purchase Invoice deleted.');
    ...
    private function uploadAndStoreFile($file, $name, $employeeId, $expiryDate)
    {
        ...
        $file->storeAs('public/purchase_invoice_docs', $filename);

        // Store only the filename in the database
        DB::table('purchase_invoice_docs')->insert([
            'name' => $name,
            'purchase_invoice_id' => $employeeId,
    ...
    public function delete_file($id)
    {
        ...
        $document = PurchaseInvoiceDoc::find($id);
        ...
        // Get the file path
        $filePath = public_path('uploads/public/purchase_invoice_docs/' . basename($document->file));
