...
class PaymentListRepository extends BaseRepository
{
    ...
    public function create($input)
    {
        DB::beginTransaction();
        try {
            // If branch_id is empty (All Branches selected), derive it from the invoice
            if (empty($input['branch_id']) && !empty($input['owner_id'])) {
                $invoice = Invoice::find($input['owner_id']);
                if ($invoice) {
                    $input['branch_id'] = $invoice->branch_id;
                }
            }
    ...
    public function getInvoices($isEdit = null)
    {
        ...
        // Return the invoices with the adjusted payment statuses and include customer and project relationships
        return Invoice::with(['payments', 'customer', 'project'])
