...
class PaymentListing extends AppBaseController
{
    ...
    public function create()
    {
        ...
        $invoices = $invoices->map(function ($invoice) {
            ...
            // Ensure we're not encoding HTML entities
            if ($invoice->customer && $invoice->customer->company_name) {
                // Decode any existing entities to prevent double encoding
                $invoice->customer->company_name = html_entity_decode($invoice->customer->company_name, ENT_QUOTES, 'UTF-8');
            }

            if ($invoice->project && $invoice->project->project_name) {
                $invoice->project->project_name = html_entity_decode($invoice->project->project_name, ENT_QUOTES, 'UTF-8');
            }
        ...
        // Add "All Branches" option to the branches array
        $usersBranches = ['' => 'All Branches'] + $usersBranches->toArray();
