    ...
    public function index(Request $request)
    {
        if ($request->ajax()) {
            try {
                $dataTable = DataTables::of((new ManageAttendanceDataTable())->get($request->all()));
                $response = $dataTable->addColumn('latest_date', function ($employee) {
                    return $employee->latest_date;
                })->make(true);

                $responseData = $response->getData(true);
                $salaryStatus = $this->manageAttendanceRepository->salaryStatus(
                    $request['month'],
                    $request['branch_id']
                );

                $responseData['salaryStatus'] = $salaryStatus;
                return response()->json($responseData);
            } catch (\Exception $e) {
                \Log::error('Timesheet AJAX Error: ' . $e->getMessage());
        ...
        // Pass both month and branch_id
        $allEmployees = $this->manageAttendanceRepository->salaryEmployees($selectedMonth, $selectedBranch);

        $usersBranches = $this->getUsersBranches();

        return view('manage_attendances.index_new', compact([
            'customers',
            'projects',
            'departments',
            'designations',
