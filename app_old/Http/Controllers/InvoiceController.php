...
class InvoiceController extends AppBaseController
{
    ...
    // dd($customers);
        ...
        // Check if invoice is draft and modify groupName accordingly
        if ($invoice->payment_status == Invoice::STATUS_DRAFT) {
            // For draft invoices, use draft view
            if ($groupName === 'invoice_details') {
                $groupName = 'draft_invoice_details';
            } else {
                // For other tabs (like payments, notes, etc.), use draft versions if they exist
                // Or keep the same group name if draft version doesn't exist
                $groupName = 'draft_' . $groupName;
            }
        }
    ...
    public function edit(Invoice $invoice)
    {
        if (
            ...
            $invoice->payment_status == Invoice::STATUS_PARTIALLY_PAID ||
        ...
        // Check if payment status is draft
        if ($invoice->payment_status == Invoice::STATUS_DRAFT) {
            // Return draft edit view
            return view('invoices.draft_edit', compact('data', 'isRound', 'invoice', 'addresses', 'projects', 'terms', 'services', 'categories', 'customers', 'usersBranches'));
        }

        // Return regular edit view for other statuses (like UNPAID)
        return view('invoices.edit', compact('data', 'isRound', 'invoice', 'addresses', 'projects', 'terms', 'services', 'categories', 'customers', 'usersBranches'));
    }
    // public function edit(Invoice $invoice)
    // {
    ...
    public function update(Invoice $invoice, UpdateInvoiceRequest $request)
    {
        ...
        try {
            ...
            if (isset($input['force_today_date']) && $input['force_today_date'] == 1) {
                $input['invoice_date'] = now()->toDateString();
            }
    ...
    public function covertToPdf(Invoice $invoice)
    {
        ...
        // Determine invoice text based on status
        $invoiceText = ($invoice->payment_status === Invoice::STATUS_DRAFT) ? 'Draft Invoice' : 'VAT Invoice';
        $invoiceTextArabic = ($invoice->payment_status === Invoice::STATUS_DRAFT) ? 'فاتورة مسودة' : 'فاتورة ضريبية';
...
<header style="width: 100%; height: 110px;">
    ...
    <div class="content-header" style="width: 100%; height: 1.80cm; margin: 0; position: relative; overflow: hidden;">
        <!-- Left: VAT -->
        ...
        <div class="content_header_title" style="margin-top: 10px; float: left; display: inline-block; width: 35%; height: 1.13cm; text-align: center; line-height: 40px; border: 1px solid #e2e2e2; background: #fff7f2; margin-left: 2.5%; margin-right: 2.5%;">
        ...
        <!-- Right: Branch - FIXED ALIGNMENT -->
        <div class="branch_name" style="display: inline-block; padding: 5px; font-size: 12pt; text-align: right; padding-top: 20px; width: 25%; float: left; box-sizing: border-box;">
...
');
        ...
        // COMPLETE FOOTER - Addresses moved closer together
...
<footer style="width: 100%; margin: 0; padding: 0; font-family: DejaVu Sans, Arial, sans-serif; background: white;">
    ...
    <div style="width: 100%; text-align: center; padding-top: 5px;">
        ...
        <div style="color: #F7996E; font-size: 10pt; font-weight: bold; text-align: center; direction: rtl; line-height: 1;">
    ...
    <div style="width: 100%; background-color: #F7996E; padding: 6px 0; margin-top: 4px; text-align: center;">
    ...
    <div style="width: 100%; padding: 3px 0; text-align: center; font-size: 8pt; color: #666; margin-top: 2px;">
        ...
        // Determine which view to use based on invoice status
        $viewName = 'invoices.invoice_pdf_arabic';
        if ($invoice->payment_status === Invoice::STATUS_DRAFT) {
            $viewName = 'invoices.draft_invoice_pdf_arabic';
        }
