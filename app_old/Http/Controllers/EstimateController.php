...
class EstimateController extends AppBaseController
{
    ...
    public function show(Estimate $estimate)
    {
        ...
        $groupName = request('group') ?? 'estimate_details';
        $settings = Setting::pluck('value', 'key');
        $isRound = $settings['is_round'] ?? 0;

        return view("estimates.views.$groupName", compact(
            'estimate',
            'isRound',
            'status',
            'priorities',
    ...
    //     // Calculate VAT (15%)
    //     $vatRate = 15; // 15% VAT
    //     $vatAmount = ($subtotal - ($estimate->discount ?? 0)) * ($vatRate / 100);

    //     // Calculate net amount after VAT
    //     $netAmountAfterVat = ($subtotal - ($estimate->discount ?? 0)) + $vatAmount;

    //     // Apply round off if enabled
    //     if ($isRound) {
    //         $netAmountAfterVat += $estimate->adjustment;
    //     }
    ...
    //         'settings' => $settings,
    //         'subtotal' => $subtotal,
    //         'vatRate' => $vatRate, // Add VAT rate to data
    //         'vatAmount' => $vatAmount, // Add VAT amount to data
    //         'netAmountAfterVat' => $netAmountAfterVat, // Add net amount after VAT
    //         'words' => $words,
    //         'wordsAr' => $wordsAr,
    //         'bank' => $bankDetails,
    ...
    public function convertToPdf(Estimate $estimate)
    {
        ...
        $netAmount = $totalTaxable + $totalVat;

        // Apply round off if enabled
        if ($isRound) {
            $netAmount += $estimate->adjustment;
        }

        // Use NET AMOUNT for words conversion (not $estimate->total_amount)
        ...
        $mpdf = new Mpdf([
            'format' => 'A4',
            'margin_top' => 45,
            'margin_bottom' => 19, // Keep original
            'margin_left' => 0,
            'margin_right' => 0,
            'margin_header' => 0,
            'margin_footer' => 0,
    ...
    ');
        ...
        // DYNAMIC FOOTER - Replacing the image footer
        $footerHtml = '
   <footer style="width: 100%; margin: 0; padding: 0; font-family: DejaVu Sans, Arial, sans-serif; background: white;">
        ...
        <!-- Email & Website (Gold Background, White Text) -->
        <div style="width: 100%; background-color: #F7996E; padding: 6px 0; margin-top: 3px; text-align: center;">
        ...
        <!-- Page Numbers -->
        <div style="text-align: start; font-size: 10px; padding-left: 10px; color: #666;">
            Page {PAGENO} of {nbpg}
        </div>
    </footer>';
