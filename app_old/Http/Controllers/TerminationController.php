...
class TerminationController extends AppBaseController
{
    ...
    public function update(Termination $termination, UpdateTerminationRequest $updateTerminationRequest)
    {
        ...
        $updateRetirement = $this->terminationRepository->update($input, $termination->id);

        activity()->performedOn($updateRetirement)
            ->causedBy(getLoggedInUser())
            ->useLog('Termination Updated')
            ->log($updateRetirement->name . ' Termination updated.');
