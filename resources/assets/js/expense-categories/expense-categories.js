...
window.renderData = function (id) {
    $.ajax({
        url: route('expense-categories.edit', id),
        type: 'GET',
        success: function (result) {
            if (result.success) {
...
$(document).on('submit', '#editForm', function (event) {
    ...
    $.ajax({
        url: route('expense-categories.update', id),
        type: 'put',
        data: $(this).serialize(),
        success: function (result) {
            if (result.success) {
...
$(document).on('click', '.delete-btn', function (event) {
    let expenseCategoryId = $(event.currentTarget).data('id');
    deleteItem(route('expense-categories.destroy', expenseCategoryId),
        '#expenseCategoriesTable', Lang.get('messages.common.expense_category'));
});
