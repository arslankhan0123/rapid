            ...
            <tbody id="itemRows">
                @foreach ($estimate->salesItems->reverse()->values() as $index => $item)
                    ...
                    <tr data-index="{{ $displayIndex - 1 }}"> <!-- Subtract 1 to make it 0-based for data-index -->
                        ...
                        <td class="p-1">
                            <input type="number" class="form-control smallInput text-right quantity" required step="any"
                                value="{{ $item['quantity'] }}" name="itemsArr[{{ $displayIndex - 1 }}][quantity]">
                        </td>
                        <td class="p-1 text-center">
                            <select name="itemsArr[{{ $displayIndex - 1 }}]['unit_id']" class="form-control"
                                style="width: 100px;">
                                @foreach ($units as $key => $value)
                                    <option value="{{ $key }}"
                                        {{ $key == ($item->unit_id ?? '') ? 'selected' : '' }}>
                                        {{ $value }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <td class="p-1">
                            <input type="number" class="form-control smallInput text-right rates" required step="any"
                                value="{{ $item['rate'] }}" name="itemsArr[{{ $displayIndex - 1 }}][rate]">
                        </td>
                        <td class="p-1">
                            <input type="number" class="form-control smallInput text-right discount" required step="any"
                    ...
                    <td style="width: 20%;">
                        ...
                        <div class="d-inline-flex align-items-center" style="float: right;">
                            ...
                            <input type="number" name="final_discount" id="totalDiscount"
                                value="{{ $estimate->discount ?? 0 }}" class="form-control d-inline-block text-right"
