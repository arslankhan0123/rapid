@extends('layouts.app')
@section('title')
    {{ __('messages.purchase-invoices.edit') }}
@endsection
@section('page_css')
...
@section('content')
    <section class="section">
        <div class="section-header">
            <h1>{{ __('messages.purchase-invoices.edit') }}</h1>
        ...
        <div class="section-body">
            ...
            <div class="card">
                {{ Form::open(['route' => ['purchase-invoices.update', $estimate->id], 'validated' => false, 'method' => 'POST', 'id' => 'editOrderForm']) }}
                @include('purchase-invoices.edit_fields_new')
                {{ Form::close() }}
            </div>
        </div>
    </section>
    @include('purchase-invoices.templates.templates')
        ...
        $(document).ready(function() {
            ...
            $(document).on('click', '.fileDeleteBtn', function(event) {
                ...
                swal({
                    ...
                    function() {
                        deleteItemAjax(route('purchase-invoices.file.delete', fileId),
                            "Document", row, null);
                    });
            ...
            function addRow(service) {
                ...
                const row = `
                    <tr data-index="${rowIndex}">
                        ...
                        <td class="p-1"><input  type="number" step="any"  class="form-control smallInput text-right quantity" required value="${globalQTY}" name="itemsArr[${rowIndex}]['quantity']"></td>
                        ...
                        <td class="p-1"><input type="number"  step="any"  class="form-control smallInput text-right rates" required value="${globalRate}"  name="itemsArr[${rowIndex}]['rate']"></td>
                        <td class="p-1">
                            <input type="number" step="any" class="form-control smallInput text-right discount" required value="0" name="itemsArr[${rowIndex}]['discount']">
                        </td>
                        <td class="p-1">
    ...
    <script>
        $(document).ready(function() {
            $('#btnSave').on('click', function() {
                ...
                $.ajax({
                    url: '{{ route('purchase-invoices.update', ['invoice' => $estimate->id]) }}', // Pass $order->id to the route
                    ...
                    success: function(response) {
                        ...
                        const url = route('purchase-invoices.index');
