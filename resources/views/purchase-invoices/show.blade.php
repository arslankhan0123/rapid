@extends('layouts.app')
@section('title')
    {{ __('messages.purchase-invoices.view') }}
@endsection
@section('page_css')
        ...
        <div class="section-header item-align-right">
            ...
            <div class="section-header-breadcrumb float-right">
                @if($estimate->status !== \App\Models\Estimate::STATUS_DECLINED && $estimate->status !== \App\Models\Estimate::STATUS_EXPIRED)
                    <a href="{{ route('purchase-invoices.edit', ['invoice' => $estimate->id]) }}"
                       class="btn btnWarning text-white mr-2 form-btn">{{ __('messages.common.edit') }}</a>
                @endif
                <a href="{{ route('purchase-invoices.index') }}" class="btn btn-primary form-btn">{{ __('messages.common.list') }}</a>
            </div>
        </div>
