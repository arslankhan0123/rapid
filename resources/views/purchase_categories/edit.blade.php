@extends('layouts.app')
@section('title')
    {{ __('messages.purchase-categories.edit') }}
@endsection
@section('page_css')
...
@section('content')
    <section class="section">
        <div class="section-header item-align-right">
            <h1>{{ __('messages.purchase-categories.edit') }}</h1>
            <div class="section-header-breadcrumb float-right">
                <div class="card-header-action mr-3 select2-mobile-margin">
                </div>
            </div>
            <div class="float-right">
                <a href="{{ route('purchase-categories.index') }}" class="btn btn-primary form-btn">
            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <div class="row">
                        ...
                        <div class="form-group col-sm-12">
                            {{ Form::label('purchase_group_id', __('messages.purchase-categories.name') . ':') }}<span
                                class="required">*</span>
                            {{ Form::select('purchase_group_id', $groups ?? [], $category->purchase_group_id??null, ['class' => 'form-control select2', 'required', 'id' => 'type_select', 'placeholder' => __('messages.placeholder.select_group')]) }}
                        </div>
...
@section('scripts')
    ...
    <script>
        $(document).on('submit', '#editForm', function(event) {
            ...
            $.ajax({
                url: route('purchase-categories.update', id),
                type: 'put',
                data: $(this).serialize(),
                success: function(result) {
                    if (result.success) {
                        displaySuccessMessage(result.message);
                        const url = route('purchase-categories.index');
