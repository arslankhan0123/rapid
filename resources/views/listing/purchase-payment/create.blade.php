                        ...
                        <div class="row">
                            ...
                            <div class="form-group col-md-6">
                                {{ Form::label('payment_mode', __('messages.invoice.invoice') . ':') }}<span
                                    class="required">*</span>
                                {{ Form::select('owner_id', $invoices->pluck('estimate_number', 'id'), null, ['class' => 'form-control select2', 'id' => 'owner_id', 'required', 'placeholder' => __('messages.invoice.invoice_number')]) }}
                            </div>
                            <div class="form-group col-md-6">
...
@section('scripts')
    ...
    <script>
        $(document).ready(function() {
            ...
            //     $.ajax({
            //         url: '{{ route('purchase-invoice-payments.list.store') }}', // The route to handle form submission
            //         type: 'POST',
            //         data: formData,
            //         success: function(response) {
            //             displaySuccessMessage(response.message);
            //             const url = route('purchase-invoice-payments.list.index');
            ...
            $('#addNewPaymentForm').on('submit', function(e) {
                ...
                let remainingAmount = parseFloat($('#remainingAmount').val());

                if (paymentAmount > remainingAmount) {
                    displayErrorMessage("Payment amount cannot be greater than the remaining balance.");
                    return false;
                }
                ...
                $.ajax({
                    url: '{{ route('purchase-invoice-payments.list.store') }}',
                    type: 'POST',
                    data: formData,
                    success: function(response) {
                        displaySuccessMessage(response.message);
