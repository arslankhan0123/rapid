@extends('layouts.app')
@section('title')
    {{ __('messages.opening-stocks.edit') }}
@endsection
@section('page_css')
...
@section('content')
    <section class="section">
        <div class="section-header">
            <h1>{{ __('messages.opening-stocks.edit') }}</h1>
        ...
        <div class="section-body">
            ...
            <div class="card">
                {{ Form::open(['route' => ['opening-stocks.update', $estimate->id], 'validated' => false, 'method' => 'POST', 'id' => 'editOrderForm']) }}
                @include('opening-stocks.edit_fields_new')
                {{ Form::close() }}
            </div>
        </div>
    </section>
            ...
            function addRow(service) {
                ...
                const row = `
                    <tr data-index="${rowIndex}">
                        ...
                        <td class="p-1"><input type="number" class="form-control smallInput text-right rates" required value="${globalRate}" readonly  name="itemsArr[${rowIndex}]['rate']"></td>
                        <td class="p-1">
    ...
    <script>
        $(document).ready(function() {
            $('#btnSave').on('click', function() {
                ...
                $.ajax({
                    url: '{{ route('opening-stocks.update', ['stock' => $estimate->id]) }}', // Pass $order->id to the route
