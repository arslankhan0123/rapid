@extends('layouts.app')
@section('title')
    {{ __('messages.opening-stocks.add') }}
@endsection
@section('page_css')
...
@section('content')
    <section class="section">
        <div class="section-header " style="height: 55px;margin-bottom:5px;">
            <h1>{{ __('messages.opening-stocks.add') }}</h1>
            <div class="section-header-breadcrumb">
                <a href="{{ route('opening-stocks.index') }}" class="btn btn-primary form-btn float-right-mobile">
        ...
        <div class="section-body">
            ...
            <div class="card">
                {{ Form::open(['route' => 'opening-stocks.store', 'validated' => false, 'id' => 'orderForm']) }}
                @include('opening-stocks.fields')
                {{ Form::close() }}
            </div>
        </div>
    </section>
    @include('opening-stocks.templates.templates')
...
@section('scripts')
    <script>
        ...
        var $units = @json($units); // Pass PHP variable to JavaScr
        ...
        $(document).ready(function() {
            ...
            $("#supplierAutocomplete").autocomplete({
                ...
                select: function(event, ui) {
                    ...
                    $("#customerId").val(selectedSupplier.id);
                    return false;
                }
            });
        ...
        $(document).ready(function() {
            ...
            $("#serviceSearch").autocomplete({
                ...
                source: function(request, response) {
                    $.ajax({
                        url: "{{ route('opening-stocks.item') }}",
                        type: "GET",
                        dataType: "json",
                        data: {
                            term: request.term // pass what user typed
                        },
            ...
            function addRow(service) {
                ...
                var globalRate = service?.cost_price ?? 0; // Get value from the input with ID 'globalRate'
                ...
                const row = `
                    <tr data-index="${rowIndex}">
                        ...
                        <td class="p-1"><input type="number" class="form-control smallInput text-right rates" readonly required value="${globalRate}"  name="itemsArr[${rowIndex}]['rate']"></td>
                        <td class="p-1">
    ...
    <script>
        $(document).ready(function() {
            $('#btnSave').on('click', function() {
                ...
                $.ajax({
                    url: '{{ route('opening-stocks.store') }}', // Route for the store method
                    type: 'POST',
                    data: formData,
                    dataType: 'json', // Expect JSON response
                    success: function(response) {
                        displaySuccessMessage(response.message);
                        const url = route('opening-stocks.index');
