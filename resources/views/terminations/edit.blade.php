...
@section('content')
    <section class="section">
        <div class="section-header item-align-right">
            <h1>{{ __('messages.terminations.edit') }}</h1>
            <div class="section-header-breadcrumb float-right">
                <div class="card-header-action mr-3 select2-mobile-margin">
                </div>
            </div>
            <div class="float-right">
                <a href="{{ route('terminations.index') }}" class="btn btn-primary form-btn">List</a>
        ...
        <div class="section-body">
            <div class="card">
                <div class="card-body">
                    ...
                    {{ Form::hidden('id', $termination->id, ['id' => 'retirement_id']) }}
                    ...
                    <div class="row">
                        <div class="form-group col-sm-12">
                            {{ Form::label('approved_date', __('messages.common.date') . ':') }}<span
                                class="required">*</span>
                            {{ Form::date('date', $termination->date ?? null, ['class' => 'form-control', 'required']) }}
                        </div>
                        <div class="form-group col-sm-12">
                        ...
                        <div class="form-group col-md-6">
                            {{ Form::label('bonus_type_id', __('messages.branches.name') . ':') }}<span
                                class="required"></span>
                            {{ Form::select('from', $usersBranches, $termination->employee?->branch_id ?? null, [
                                'class' => 'form-control',
                                'id' => 'from_branch',
                        ...
                        {{-- <div class="form-group col-md-6">
                            {{ Form::label('title', __('messages.terminations.name') . ':') }}<span
                                class="required">*</span>
                            {{ Form::text('name', $termination->name, ['class' => 'form-control', 'required', 'id' => 'designation_name', 'autocomplete' => 'off', 'placeholder' => __('messages.designations.name')]) }}
                        </div> --}}
                        <div class="form-group col-sm-12 mb-0">
                            {{ Form::label('description', __('messages.assets.category_description') . ':') }}
                            {{ Form::textarea('description', $termination->description, ['class' => 'form-control summernote-simple', 'id' => 'createDescription']) }}
                        </div>
                    </div>
                    <div class="form-group col-sm-12 mb-0">
                        <div class="form-check form-switch ">
                            {{ Form::label('status', __('messages.terminations.status') . ':') }}
                            {{ Form::checkbox('status', true, $termination->status, ['class' => 'form-check-input ml-4', 'id' => 'statusSwitchEdit']) }}
                        </div>
                    </div>
    ...
    <script>
        ...
        $(document).on('submit', '#editForm', function(event) {
            ...
            if (departmentSelect === '' || departmentSelect === null) {
                displayErrorMessage('{{ __('messages.terminations.select_department') }}');
                return false;
            }
            $.ajax({
                url: route('terminations.update', id),
                type: 'put',
                data: $(this).serialize(),
        ...
        $(document).ready(function() {
            ...
            if (defaultEmployeeId) {
                ...
                if (defaultEmployee) {
                    ...
                    $('#employeeDepartment').text('{{ __('messages.allowances.employee_department') }} : ' + (
                        defaultEmployee.department ? defaultEmployee.department.name : ''));
                    // $('#employeeSubDepartment').text(
                    //     '{{ __('messages.allowances.employee_sub_department') }} : ' + (defaultEmployee.sub_department?defaultEmployee.sub_department.name:''));
