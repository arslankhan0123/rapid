...
<body>
    ...
    <main>
        ...
        <div class="content_header">
            ...
            <div class="middle">
                <h1 style="font-size: 20pt;">Sample Receiving</h1>
        ...
        <div class="content_info">
            <table border="1" cellspacing="0" cellpadding="5"
                style="width: 98%; border-collapse: collapse; font-family: Arial, sans-serif; font-size: 14px;margin-left:1%;">
                <tr>
                    <td style="width: 20%; font-weight: bold;">Branch</td>
                    <td style="width: 30%;">{{ $category->branch?->name ?? '' }}</td>
                    <td style="width: 20%; font-weight: bold;">Client Name</td>
                    <td style="width: 30%;">{{ $category->client_name ?? '' }}</td>
                </tr>
                <tr>
                    <td style="font-weight: bold;">Client Reference</td>
                    <td>{{ $category->client_reference ?? '' }}</td>
                    <td style="font-weight: bold;">Type of Sample</td>
                    <td>{{ $category->type_of_sample ?? '' }}</td>
                </tr>
                <tr>
                    <td style="font-weight: bold;">Number of Sample</td>
                    <td>{{ $category->number_of_sample ?? '' }}</td>
                    <td style="font-weight: bold;">Date</td>
                    <td>{{ $category->date ? \Carbon\Carbon::parse($category->date)->format('d-m-Y') : '' }}</td>
                </tr>
                <tr>
                    <td style="font-weight: bold;">Time</td>
                    <td>{{ $category->time ? \Carbon\Carbon::parse($category->time)->format('h:i A') : '' }}</td>
                    <td style="font-weight: bold;">Required Tests</td>
                    <td>{{ $category->required_tests ?? '' }}</td>
                </tr>
                <tr>
                    <td style="font-weight: bold;">Section</td>
                    <td>{{ $category->category?->name ?? '' }}</td>
                    <td style="font-weight: bold;">Delivered By</td>
                    <td>{{ $category->deliveredBy?->name ?? '' }}</td>
                </tr>
                <tr>
                    <td style="font-weight: bold;">Received By</td>
                    <td>{{ $category->receivedBy?->name ?? '' }}</td>
                    <td style="font-weight: bold;">Branch Company</td>
                    <td>{{ $category->branch?->company_name ?? '' }}</td>
                </tr>
            </table>
        </div>
    </main>
