@extends('layouts.app')
@section('title')
    {{ __('messages.supplier_statements.name') }}
@endsection
@section('page_css')
...
@section('content')
    <section class="section">
        <div class="section-header item-align-right">
            <h1>{{ __('messages.supplier_statements.name') }}</h1>
                    ...
                    @can('export_statement')
                        <div class="dropdown">
                            ...
                            <div class="dropdown-menu " aria-labelledby="exportDropdown" style="width: 50px;">
                                ...
                                <a class="dropdown-item" href="#" id="export-csv">XLS</a>
                            </div>
                        </div>
                    @endcan
                </div>
                ...
                <div class="card-body" style="padding-top:10px !important;padding-bottom:10px !important;">
                    ...
                    <div class="row justify-content-end">
                        <div class="form-group col-sm-12 col-md-2">
                            {{ Form::label('from_date', __('messages.supplier_statements.from') . ':') }}
                            <input type="date" id="from_date" name="from_date" class="form-control">
                        </div>

                        <div class="form-group col-sm-12 col-md-2">
                            {{ Form::label('to_date', __('messages.supplier_statements.to') . ':') }}
                            <input type="date" id="to_date" name="to_date" class="form-control">
                        </div>

                        <div class="form-group col-sm-12 col-md-2">
                            {{ Form::label('employee_id', 'Select Supplier' . ':') }}
                            {{ Form::select('customer_id', $suppliers, null, ['class' => 'form-control', 'required', 'id' => 'customer_select']) }}
                        </div>
                    </div>
        ...
        <div class="section-body">
            <div class="card">
                <div class="card-body">
                    ...
                    @include('supplier_statements.table')
                </div>
            </div>
        </div>
    </section>
...
@section('scripts')
    ...
    <script>
        let tbl = $('#designationTable').DataTable({
            ...
            ajax: {
                url: route('supplier-statement.index'),
                data: function(d) {
            ...
            footerCallback: function(row, data, start, end, display) {
                ...
                var totalBalance = totalReceived - totalAmount;
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#export-pdf').on('click', function(e) {
                ...
                var exportUrl = "{{ route('supplier-statement.export') }}" +
                    "?type=pdf" +
