@extends('layouts.app')
@section('title')
    {{ __('messages.company_loans.edit_company_loan') }}
@endsection
@section('page_css')
...
@section('content')
    <section class="section">
        <div class="section-header">
            <h1>{{ __('messages.company_loans.edit_company_loan') }}</h1>
        ...
        <div class="section-body">
            <div class="card">
                <div class="card-body">
                    ...
                    {!! Form::open([
                        'route' => ['company-loans.update', $companyLoan->id],
                        'id' => 'editCompanyLoanForm',
                        'method' => 'put',
                    ]) !!}
...
@section('scripts')
    <script>
        ...
        let companyLoanId = "{{ $companyLoan->id }}";
        let companyLoanType = "{{ $companyLoan->type }}";
        let companyLoanCustomerId = "{{ $companyLoan->customer_id }}";
        let companyLoanSupplierId = "{{ $companyLoan->supplier_id }}";
        let companyLoanName = "{{ $companyLoan->name }}";
    </script>
    <script>
            ...
            function calculateDays() {
                ...
                if (receivedDate && refundDate) {
                    ...
                    if (diffTime >= 0) {
                        ...
                        alert("Refund date cannot be earlier than received date!");
                        $('#loan_refund_date').val('');
                        $('#number_of_days').val('');
                    }
                }
            }
