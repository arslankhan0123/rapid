@extends('layouts.app')
@section('title')
    {{ __('messages.job_recruiters.add') }}
@endsection
@section('page_css')
...
@section('content')
    <section class="section">
        <div class="section-header item-align-right">
            <h1>{{ __('messages.job_recruiters.add') }}</h1>
            <div class="section-header-breadcrumb float-right">
                <div class="card-header-action mr-3 select2-mobile-margin">
                </div>
            </div>
            <div class="float-right">
                <a href="{{ route('job-recruiters.index') }}" class="btn btn-primary form-btn">{{ __('messages.job_recruiters.list') }} </a>
            </div>
        </div>
                        ...
                        <div class="modal-body">
                            ...
                            <div class="row">
                                <div class="form-group col-sm-12">
                                    {{ Form::label('title', __('messages.job_recruiters.name') . ':') }}<span
                                        class="required">*</span>
                                    {{ Form::text('recruiter_name', null, ['class' => 'form-control', 'required', 'id' => 'designation_name', 'autocomplete' => 'off']) }}
                                </div>
...
@section('scripts')
    ...
    <script>
        let departmentNewCreateUrl = route('job-recruiters.store');
            ...
            $.ajax({
                ...
                success: function(result) {
                    if (result.success) {
                        ...
                        const url = route('job-recruiters.index', );
