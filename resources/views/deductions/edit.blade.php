...
@section('content')
    <section class="section">
        <div class="section-header item-align-right">
            <h1>{{ __('messages.deductions.edit') }}</h1>
        ...
        <div class="section-body">
            <div class="card">
                <div class="card-body">
                    ...
                    {{ Form::hidden('id', $deduction->id, ['id' => 'deduction_id']) }}
                    <div class="modal-body">
                        <div class="alert alert-danger d-none" id="validationErrorsBox"></div>
                        <div class="row">
                            {{-- {{dd($deduction->month)}} --}}
                            <div class="form-group col-sm-12">
                                {{ Form::label('month', 'Date') }}<span class="required">*</span>
                                {{ Form::date('date', $deduction->date, ['class' => 'form-control', 'required']) }}
                            </div>
                            <div class="form-group col-sm-12">
                            ...
                            <div class="form-group col-md-6">
                                {{ Form::label('bonus_type_id', __('messages.branches.name') . ':') }}<span
                                    class="required"></span>
                                {{ Form::select('from', $usersBranches, $deduction->employee?->branch_id ?? null, [
                                    'class' => 'form-control',
                                    'id' => 'from_branch',
                            ...
                            <div class="form-group col-md-6">
                                {{ Form::label('deduction_type_id', __('messages.deductions.type') . ':') }}<span
                                    class="required">*</span>
                                {{ Form::select('deduction_type_id', $types, $deduction->deduction_type_id ?? null, [
                                    'class' => 'form-control',
                                    'required',
                                    'id' => 'type_select',
                                    'placeholder' => __('messages.deductions.type'),
                                ]) }}
                            </div>
                            <div class="form-group col-sm-12">
                                {{ Form::label('title', __('messages.deductions.amount') . ':') }}<span
                                    class="required">*</span>
                                {{ Form::number('amount', $deduction->amount, ['class' => 'form-control', 'required', 'id' => 'bonus_name', 'autocomplete' => 'off']) }}
                            </div>
                            <div class="form-group col-sm-12 mb-0">
                                {{ Form::label('description', __('messages.deductions.description') . ':') }}
                                {{ Form::textarea('description', $deduction->description, ['class' => 'form-control summernote-simple', 'id' => 'createDescription']) }}
                            </div>
                        </div>
...
@section('scripts')
    ...
    <script>
        ...
        $(document).on('submit', '#editForm', function(event) {
            ...
            $.ajax({
                url: route('deductions.update', id),
                type: 'put',
                data: $(this).serialize(),
                success: function(result) {
                    if (result.success) {
                        displaySuccessMessage(result.message);
                        const url = route('deductions.index', );
