@extends('layouts.app')
@section('title')
    {{ __('messages.purchase-orders.name') }}
@endsection
@section('page_css')
...
@section('content')
    <section class="section">
        <div class="section-header">
            <h1>{{ __('messages.purchase-orders.name') }}</h1>
...
@section('scripts')
    ...
    <script>
        ...
        $(document).on('click', '.edit-btn', function(event) {
            ...
            const url = route('purchase-orders.edit', did);
            window.location.href = url;
        });

        $(document).on('click', '.delete-btn', function(event) {
            let assetCateogryId = $(event.currentTarget).data('id');
            deleteItem(route('purchase-orders.destroy', assetCateogryId), '#designationTable',
                '{{ __('messages.purchase-orders.name') }}');
        });
    </script>
    <script>
        ...
        // Define permissions
        var permissions = {
            updateItem: "{{ auth()->user()->can('update_purchase_orders') ? 'true' : 'false' }}",
            deleteItem: "{{ auth()->user()->can('delete_purchase_orders') ? 'true' : 'false' }}",
            viewItem: "{{ auth()->user()->can('view_purchase_orders') ? 'true' : 'false' }}"
        };
        ...
        function renderActionButtons(id) {
            ...
            if (permissions.updateItem === 'true') {
                let editUrl = `{{ route('purchase-orders.edit', ':id') }}`;
            ...
            if (permissions.viewItem === 'true') {
                let viewUrl = `{{ route('purchase-orders.view', ':id') }}`;
