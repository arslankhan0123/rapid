    ...
    <div class="row">
        ...
        <div class="form-group col-sm-4">
            ...
            <p>
                ...
                <input type="hidden" class="leadWebsite" value="{{ !empty($lead->website)?$lead->website:null }}"
                       hidden>
            </div>
            <div class="form-group col-sm-4">
            ...
            <div class="form-group col-sm-4">
                ...
                <p>{{ !empty($lead->country) ? $lead->leadCountry->name : __('messages.common.n/a') }}</p>
            ...
            <div class="form-group col-sm-4">
                ...
                @if(isset($lead->date_contacted))
                    <p>{{ \Carbon\Carbon::parse($lead->date_contacted)->translatedFormat('jS M, Y H:i A') }}</p>
                @else
            ...
            <div class="form-group col-sm-4">
                ...
                <p><span data-toggle="tooltip" data-placement="right"
                         title="{{ \Carbon\Carbon::parse($lead->created_at)->translatedFormat('jS M, Y') }}">{{ $lead->created_at->diffForHumans() }}</span>
                </p>
            </div>
            <div class="form-group col-sm-4">
                {{ Form::label('updated_at', __('messages.common.last_updated').':') }}
                <p><span data-toggle="tooltip" data-placement="right"
                         title="{{ \Carbon\Carbon::parse($lead->updated_at)->translatedFormat('jS M, Y') }}">{{ $lead->updated_at->diffForHumans() }}</span>
                </p>
            </div>
    </div>
    <div class="row">
