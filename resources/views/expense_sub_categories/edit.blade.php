@extends('layouts.app')
@section('title')
    {{ __('messages.expense_sub_categories.edot') }}
@endsection
@section('page_css')
...
@section('content')
    <section class="section">
        <div class="section-header item-align-right">
            <h1>{{ __('messages.expense_sub_categories.edit') }}</h1>
        ...
        <div class="section-body">
            <div class="card">
                <div class="card-body">
                    ...
                    {{ Form::hidden('id', $category->id, ['id' => 'sub_department_id']) }}
                    <div class="alert alert-danger d-none" id="validationErrorsBox"></div>
                    <div class="row">
                        <div class="form-group col-sm-12">
                            {{ Form::label('title', __('messages.expense_sub_categories.title') . ':') }}<span
                                class="required">*</span>
                            {{ Form::text('name', $category->name, ['class' => 'form-control', 'required', 'id' => 'designation_name', 'autocomplete' => 'off']) }}
                        </div>
                        <div class="form-group col-sm-12">
                            {{ Form::label('department_id', __('messages.expense_sub_categories.select_expense_category') . ':') }}<span
                                class="required">*</span>
                            {{ Form::select('expense_category_id', $expenseCategories ?? [], $category->expense_category_id ?? null, ['class' => 'form-control', 'id' => 'expense_category_select']) }}
                        </div>
...
@section('scripts')
    ...
    <script>
        ...
        $(document).on('submit', '#editForm', function(event) {
            ...
            $.ajax({
                url: route('expense_sub_categories.update', id),
                type: 'put',
                data: $(this).serialize(),
