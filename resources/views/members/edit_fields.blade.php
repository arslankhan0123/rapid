<div class="row">
    ...
    <div class="form-group col-sm-3">
        ...
        {{ Form::select('employee_id', $employees->pluck('name', 'id'), null, ['id' => 'employeeID', 'class' => 'form-control', 'placeholder' => __('messages.placeholder.select_employee')]) }}
    </div>
</div>
...
<div class="row">
    <div class="form-group col-sm-6">
        ...
        {{ Form::email('email', $member->email, ['class' => 'form-control', 'required', 'autocomplete' => 'off', 'placeholder' => __('messages.common.email')]) }}
    </div>

    {{-- Add Password Fields for Edit --}}
    <div class="form-group col-sm-3">
        ...
        <small class="form-text text-muted">{{ __('messages.member.password_leave_blank') }}</small>
    </div>
    <div class="form-group col-sm-3">
...
<div class="row">
    <div class="form-group col-sm-6">
        ...
        {{ Form::tel('phone', $member->phone, ['class' => 'form-control', 'required', 'id' => 'phoneNumber', 'onkeyup' => 'if (/\D/g.test(this.value)) this.value = this.value.replace(/\D/g,"")']) }}
...
<div class="row mb-3">
    <div class="col-md-12">
        {{ Form::label('branches', __('messages.branches.branches')) }}
        {{ Form::select('branches[]', $branches ?? [], $branchIds ?? [], ['class' => 'form-control select2', 'multiple' => 'multiple']) }}
    </div>
</div>
...
<div class="row">
    ...
    <div class="form-group col-sm-6">
        ...
        <div class="custom-control custom-checkbox">
            <input type="checkbox" class="custom-control-input" id="isEditStaffMember" name="staff_member"
                value="1" {{ $member->staff_member == 1 ? 'checked' : '' }}>
            <label class="custom-control-label"
                for="isEditStaffMember">{{ __('messages.member.staff_member') }}</label>
        </div>
        <div class="custom-control custom-checkbox">
            <input type="checkbox" class="custom-control-input" id="isEditSendWelcomeEmail" name="send_welcome_email"
                value="1" {{ $member->send_welcome_email == 1 ? 'checked' : '' }}>
            <label class="custom-control-label" for="isEditSendWelcomeEmail">
        ...
        <div class="row">
            ...
            <div class="col-2 pl-0 mt-1">
                <img id='logoPreview' class="img-thumbnail thumbnail-preview"
                    src="{{ isset($member->employee->image) ? asset('uploads/public/employee_images/' . $member->employee->image) : (count($member->media) > 0 ? $member->image_url : asset('assets/img/infyom-logo.png')) }}">
