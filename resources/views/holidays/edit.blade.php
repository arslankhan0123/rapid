...
@section('content')
    <section class="section">
        <div class="section-header item-align-right">
            <h1>{{ __('messages.holidays.edit') }}</h1>
            ...
            <div class="card">
                <div class="card-body">
                    <div class="modal-content">
                        ...
                        {{ Form::hidden('id', $holiday->id, ['id' => 'holiday_id']) }}
                            ...
                            <div class="row">
                                <div class="form-group col-sm-12">
                                    {{ Form::label('name', __('messages.holidays.holiday_name') . ':') }}<span
                                        class="required">*</span>
                                    {{ Form::text('name', $holiday->name ?? null, ['class' => 'form-control', 'required', 'id' => 'holiday_name', 'autocomplete' => 'off']) }}
                                </div>

                                <div class="form-group col-sm-12">
                                    {{ Form::label('from_date', __('messages.holidays.from_date') . ':') }}<span
                                        class="required">*</span>
                                    {{ Form::date('from_date', $holiday->from_date, ['class' => 'form-control', 'required', 'id' => 'from_date']) }}
                                </div>

                                <div class="form-group col-sm-12">
                                    {{ Form::label('end_date', __('messages.holidays.end_date') . ':') }}<span
                                        class="required">*</span>
                                    {{ Form::date('end_date', $holiday->end_date, ['class' => 'form-control', 'required', 'id' => 'end_date']) }}
                                </div>

                                <div class="form-group col-sm-12">
                                    {{ Form::label('total_days', __('messages.holidays.total_days') . ':') }}
                                    {{ Form::text('total_days', $holiday->days_count ?? null, ['class' => 'form-control', 'required', 'id' => 'total_days', 'disabled']) }}
                                </div>
                            </div>
