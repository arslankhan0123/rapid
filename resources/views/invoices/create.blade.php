...
@section('scripts')
    ...
    <script>
        $(document).ready(function() {
            ...
            //     projectSelect.empty();
            //     projectSelect.append('<option value="">' +
            //         "{{ __('messages.placeholder.select_project') }}" + '</option>');

            //     // Filter projects based on branch ID and populate the project select
            //     $.each(allProjects, function(id, project) {
            //         if (project.branch_id == branchId) {
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            function addRow() {
                $('#itemRows .sales-items-empty-placeholder').remove();
            ...
            $(document).on('change', '.categorySelect', function() {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        var allowed = new Set(tmpSelectedProject.services.map(function(s) {
                            return String(s.service_id);
                        }));
                        list.forEach(function(service) {
                            if (allowed.has(String(service.id))) {
            ...
            $(document).on('click', '.remove-row', function() {
                $(this).closest('tr').remove();
                if ($('#itemRows tr').not('.sales-items-empty-placeholder').length === 0) {
                    rowIndex = 0;
                    addRow();
                }
                calculateTotals();
            ...
            function calculateTotals() {
                ...
                $('#itemRows tr').not('.sales-items-empty-placeholder').each(function() {
                    let row = $(this);
                    let quantity = parseFloat(row.find('.quantity').val()) || 0;
                    let rate = parseFloat(row.find('.rates').val()) || 0;
        ...
        $(document).ready(function() {
            ...
            function updateServiceDropdown(projectId) {
                ...
                if (selectedProject) {
                    ...
                    tmpSelectedProject = selectedProject;
                    tmpServices = selectedProject.services; // Get the services for the selected project
                    // console.log(tmpServices); // For debugging
