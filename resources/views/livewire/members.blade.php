<div class="row">
    ...
    @forelse($members as $member)
        <div class="col-12 col-md-6 col-lg-4 col-xl-4 extra-large">
            <div
                class="livewire-card card {{ $loop->odd ? 'card-primary' : 'card-dark' }} shadow mb-5 rounded user-card-view user-card-mbl-height">
                        ...
                        <div class="justify-content-between d-flex">
                            <div class="user-card-name pb-1">
                                <a href="{{ url('admin/members/' . $member->id) }}" class="anchor-underline">
                                    <h4>{{ Str::limit(html_entity_decode($member->first_name), 12, '...') }}</h4>
                                </a>
                            </div>
                                    ...
                                    <div class="dropdown-list-content dropdown-list-icons">
                                        @can('update_users')
                                            <a href="{{ route('members.edit', $member->id) }}"
                                                ...
                                                data-id="{{ $member->id }}"><i
                                                    class="fas fa-edit mr-2 card-edit-icon"></i>
                                                {{ __('messages.common.edit') }}
                                            </a>
                                        @endcan
                                        @if (getLoggedInUserId() != $member->id)
                                            @can('delete_users')
                        ...
                        <div class="card-user-email pt-1 mb-1">
                            ...
                            @else
                                <span data-toggle="tooltip"
                                    title="{{ __('messages.member.email_is_not_verified') }}"><i
                                        class="fas fa-times-circle email-not-verified"></i></span>
                            @endif
                        </div>
                        <div class="mr-3 mt-2">
                            <span class="badge badge-primary">{{ $member->projects_count }}</span>
                ...
                <div class="card-body d-flex align-items-center pt-0 pl-3 ml-2">
                    @if ($member->id != getLoggedInUserId())
                        <div class="mt-2 member-card-toggle card-toggle-mr">
                            <label class="custom-switch pl-0" data-placement="bottom" data-toggle="tooltip"
                                title="{{ __('messages.common.status') }}">
                                <input type="checkbox" name="is_enable" class="custom-switch-input is-administrator"
                                    data-id="{{ $member->id }}" value="1" data-class="is_enable"
                                    {{ $member->is_enable ? 'checked' : '' }}>
                    ...
                    @if (empty($member->email_verified_at))
                        <div class="ml-auto mt-1 member-card-toggle">
                            <button class="btn btn-danger btn-sm p-0 pl-1 pr-1 email-verify-btn"
                                data-id="{{ $member->id }}" data-toggle="tooltip"
                                title="{{ __('messages.member.email_verify') }}">
                                <i class="fas fa-envelope font-size-12px"></i></button>
                            <button class="btn btn-info btn-sm p-0 pl-1 pr-1 email-btn" data-id="{{ $member->id }}"
                                data-toggle="tooltip" title="{{ __('messages.member.resend_email_verification') }}"><i
                                    class="fas fa-sync font-size-12px"></i></button>
                        </div>
                    @else
                        <div class="ml-auto mt-1 member-card-toggle">
                            @if (!$member->is_admin)
                                @if (!getLoggedInUser()->hasRole('client') && $member->is_enable)
                                    <a href="{{ route('impersonate', $member->id) }}">
                                        <button class="btn btn-info btn-sm p-0 pl-1 pr-1 email-verified-btn"
                            ...
                            <button class="btn btn-success btn-sm p-0 pl-1 pr-1 email-verified-btn"
                                data-toggle="tooltip" title="{{ __('messages.member.email_verified') }}">
                                <i class="fas fa-envelope font-size-12px"></i></button>
                        </div>
                    @endif
                </div>
    ...
    @empty
        <div class="mt-0 mb-5 col-12 d-flex justify-content-center mb-5 rounded">
            <div class="p-2">
                ...
                @else
                    <p class="text-dark">{{ __('messages.member.no_member_found') }}</p>
                @endif
            </div>
        </div>
    @endforelse
            ...
            <div class="row paginatorRow">
                <div class="col-lg-2 col-md-6 col-sm-12 pt-2">
                    <span class="d-inline-flex">
                        ...
                        <span class="font-weight-bold ml-1 mr-1">{{ $members->lastItem() }}</span>
