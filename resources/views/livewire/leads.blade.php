            ...
            <div class="row justify-content-md-center text-center mb-4">
                <div class="owl-carousel owl-theme">
                    @foreach ($data as $status)
                        <div class="item">
                            <div class="ticket-statistics mx-auto"
                                {{ $inStyle }}="{{ $bgColor }} {{ $status->color }}">
                        ...
                        <div class="ml-2 w-100 mb-auto">
                            <div class="justify-content-between d-flex">
                                <div class="d-inline-block lead-card-name">
                                    <a href="{{ url('admin/leads/' . $lead->id) }}" class="anchor-underline">
                                        ...
                                        <span class="text-grey">({{ html_entity_decode($lead->priority) }})</span>
                                    </a>
                                </div>
                            ...
                            <div class="card-body pt-1 pl-0 pb-0">
                                <div class="line-height-20px">
                                    ...
                                    <span>{{ Str::limit(html_entity_decode($lead->leadSource->name), 15, '...') }}</span>
                                </div>
                            </div>
                            <div class="card-body pt-0 pl-0 pb-0">
                                @if (!empty($lead->estimate_budget))
                                    <div class="line-height-20px">
                                        <label class="font-weight-bold">{{ __('messages.lead.estimate_budget') }}:
                                        </label>
                                        <span><i class="{{ getCurrencyClass() }}"></i>
                                            {{ number_format($lead->estimate_budget, 2) }}</span>
                                    </div>
                                @endif
                            </div>
                            @php
                            ...
                            <div class="card-body pt-0 pl-0 pb-3 d-flex pr-0">
                                <div class="pt-2 mobile-margin-right">
                                ...
                                <div class="author-box-left pl-0 ml-auto">
                                    @if (!empty($lead->assignedTo->image_url))
                                        <a href="{{ url('admin/members/' . $lead->assign_to) }}"><img alt="image"
                                                width="50" src="{{ $lead->assignedTo->image_url }}"
                                                class="rounded-circle lead-avatar-image uAvatar"
                                                title="{{ html_entity_decode($lead->assignedTo->full_name) }}"></a>
                                    @endif
                                </div>
                            </div>
                        </div>
                ...
                <div class="row paginatorRow">
                    <div class="col-lg-2 col-md-6 col-sm-12 pt-2">
                        <span class="d-inline-flex">
                            ...
                            <span class="font-weight-bold ml-1 mr-1">{{ $leads->lastItem() }}</span>
