            ...
            <div class="livewire-card card card-{{ \App\Models\Project::CARD_COLOR[$project->status] }} shadow mb-5 rounded project-card-height">
                ...
                <div class="card-body pt-0 pl-3">
                    ...
                    <div class="float-left w-100 mt-1">
                        <span class="{{(now() > \Carbon\Carbon::parse($project->deadline)->toDate())?'text-danger':'' }}"
                              title="{{ __('messages.project.deadline') }}">{{ Carbon\Carbon::parse($project->deadline)->translatedFormat('jS M, Y ') }}</span>
                    </div>
                </div>
            </div>
        </div>
