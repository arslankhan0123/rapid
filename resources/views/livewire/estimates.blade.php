<div>
    <div class="row">
        <div class="mt-0 mb-3 col-12 d-flex justify-content-end search-display-block">
            @if (!empty($customer))
                <div class="mt-2">
                    {{ Form::select('status', $estimateStatus, $statusFilter, ['id' => 'estimateFilterStatus', 'class' => 'form-control', 'placeholder' => __('messages.placeholder.select_status')]) }}
                </div>
            @endif
            <div class="p-2">
        ...
        @forelse($estimates as $estimate)
            <div class="col-12 col-md-6 col-xl-3 col-xlg-4">
                <div
                    class="estimate-card card card-{{ \App\Models\Estimate::STATUS_COLOR[$estimate->status] }} shadow mb-5 rounded hover-card estimate-card-border height-210">
                    <div
                        ...
                        <div class="d-flex w-100 justify-content-between">
                            <a href="{{ url('admin/estimates', $estimate->id) }}"
                                ...
                                <h4 class="invoice-clients invoice_title">
                                    ...
                                    {{ Str::limit(html_entity_decode($estimate->customer_name . ' (' . $estimate->estimate_number . ')'), 35, '...') }}
                                </h4>
                        ...
                        <div class="d-table w-100">
                            <div>
                                <i class="fas fa-street-view"></i><span class="text-decoration-none"
                                    ...
                                    {{ Str::limit(html_entity_decode($estimate->customer_name), 15) }}</span>
                            </div>
                            <span class="d-table-row w-100">

                                <span
                                    class="mt-mobile-2 badge-{{ App\Models\Estimate::STATUS_COLOR[$estimate->status] }} badge text-uppercase">{{ App\Models\Estimate::STATUS[$estimate->status] }}</span>
                            </span>
                            @if (!empty($estimate->estimate_expiry_date))
                                <span
                                    class="d-table-row text-nowrap w-100 {{ now() > Carbon\Carbon::parse($estimate->estimate_expiry_date) ? 'text-danger' : '' }}">
                                    {{ Carbon\Carbon::parse($estimate->estimate_expiry_date)->translatedFormat('jS M, Y') }}
                                </span>
                            @endif
                        </div>
                    </div>
        ...
        @empty
            <div class="mt-0 mb-5 col-12 d-flex justify-content-center  mb-5 rounded">
                <div class="p-2">
                    ...
                    @else
                        <p class="text-dark">{{ __('messages.estimate.no_estimate_found') }}</p>
                    @endif
                </div>
            </div>
        @endforelse
                ...
                <div class="row paginatorRow">
                    <div class="col-lg-2 col-md-6 col-sm-12 pt-2">
                        <span class="d-inline-flex">
                            ...
                            <span class="font-weight-bold ml-1 mr-1">{{ $estimates->lastItem() }}</span>
