@extends('layouts.app')
@section('title')
    {{ __('messages.purchase-returns.add') }}
@endsection
@section('page_css')
...
@section('content')
    <section class="section">
        <div class="section-header " style="height: 55px;margin-bottom:5px;">
            <h1>{{ __('messages.purchase-returns.add') }}</h1>
            <div class="section-header-breadcrumb">
                <a href="{{ route('purchase-returns.index') }}" class="btn btn-primary form-btn float-right-mobile">
        ...
        <div class="section-body">
            ...
            <div class="card">
                {{ Form::open(['route' => 'purchase-returns.store', 'validated' => false, 'id' => 'orderForm']) }}
                @include('purchase-returns.fields')
                {{ Form::close() }}
            </div>
        </div>
    </section>
    @include('purchase-returns.templates.templates')
...
@section('scripts')
        ...
        $(document).ready(function() {
            ...
            $("#supplierAutocomplete").autocomplete({
                ...
                select: function(event, ui) {
                    ...
                    $("#customerId").val(selectedSupplier.customer?.id ?? null);
    ...
    <script>
        $(document).ready(function() {
            $('#btnSave').on('click', function() {
                ...
                var form = document.getElementById('orderForm'); // get form by ID
                var formData = new FormData(form); // create FormData from the form element

                startLoader();
                // Perform the AJAX call
                $.ajax({
                    url: '{{ route('purchase-returns.store') }}', // Route for the store method
                    type: 'POST',
                    data: formData,
                    ...
                    success: function(response) {
                        ...
                        const url = route('purchase-returns.index');
    ...
    <script>
        $(document).ready(function() {
            $('#purchaseInvoiceNumber').on('keyup', function(e) {

                let invoiceId = $('#purchaseInvoiceNumber').val().trim();

                if (invoiceId !== '') {
                    $.ajax({
                        url: "{{ route('purchase-returns.invoice.view', ':invoice') }}".replace(
                            ':invoice',
                            invoiceId),
                        type: 'GET',
                        dataType: 'json',
                        success: function(response) {
                            if (response.success) {
                                ...
                                $("#estimateNumber").val(response.data.po_number ?? '');
                                if (response.data.estimate_date) {
                                    const dateParts = response.data.estimate_date.split('T')[0]
                                        .split('-');
                                    const formattedDate =
                                        `${dateParts[2]}-${dateParts[1]}-${dateParts[0]}`;
                                    $("#estimate_date").val(formattedDate);
                                } else {
                                    $("#estimate_date").val('');
                                }

                                $("#supplierAutocomplete").val(response.data.customer_name ??
                                    '');
                                $("#customerId").val(response.data.customer_id ?? null);
                                $(".country").val(response.data.customer?.supplier_country
                                    ?.name ?? '');
