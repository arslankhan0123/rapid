@extends('layouts.app')
@section('title')
    {{ __('messages.purchase-returns.edit') }}
@endsection
@section('page_css')
...
@section('content')
    <section class="section">
        <div class="section-header">
            <h1>{{ __('messages.purchase-returns.edit') }}</h1>
        ...
        <div class="section-body">
            ...
            <div class="card">
                {{ Form::open(['route' => ['purchase-returns.update', $estimate->id], 'validated' => false, 'method' => 'POST', 'id' => 'editOrderForm']) }}
                @include('purchase-returns.edit_fields_new')
                {{ Form::close() }}
            </div>
        </div>
    </section>
    ...
    <script>
        $(document).ready(function() {
            $('#btnSave').on('click', function() {
                ...
                var form = document.getElementById('editOrderForm'); // get form by ID
                var formData = new FormData(form); // create FormData from the form element
                startLoader();
                // Perform the AJAX call
                $.ajax({
                    url: '{{ route('purchase-returns.update', ['return' => $estimate->id]) }}', // Pass $order->id to the route
            ...
            $(document).on('click', '.fileDeleteBtn', function(event) {
                ...
                swal({
                    ...
                    function() {
                        deleteItemAjax(route('purchase-returns.file.delete', fileId),
                            "Document", row, null);
                    });
                        ...
                        success: function(response) {
                            if (response.success) {
                                ...
                                $(".country").val(response.data.customer?.supplier_country
                                    ...
                                    $("#branch_id").val(response.data.branch_id).trigger(
                                        'change');
                                }
