...
class EmployeeSalaryController extends AppBaseController
{
    ...
    public function payslip(SalarySheet $salarySheet)
    {
        ...
        $allowancesRaw = Allowance::with('allowanceTypes')
            ->where('employee_id', $salarySheet->employee_id)
            ->whereYear('date', $salaryMonth->year)
            ->get();

        $allowancesTotal = round($allowancesRaw->sum('amount'), 2); // Raw HRA total for the year

        $allowances = $allowancesRaw
            ->groupBy('allowanceTypes.name')
            ->map(function ($group, $key) use ($salarySheet) {
                $typeTotal = $group->sum('amount');
                $typeDeduction = ($typeTotal / 30 / 8) * ($salarySheet->absence_hours ?? 0);
                return [
                    'type'         => $key,
                    'raw_amount'   => round($typeTotal, 2),
                    'total_amount' => round(max(0, $typeTotal - $typeDeduction), 2), // After absence deduction
                ];
            });
        ...
        return view('employee_salaries.payslip', compact('salarySheet', 'words', 'allowances', 'allowancesTotal', 'banks', 'settings','comLogoPath'));
    }
    ...
    public function payslipPDF(SalarySheet $salarySheet, $overtime_status = null, $isFullSalary = null)
    {
        ...
        if ($overtime_status) {
            ...
            $pdf = PDF::loadView('employee_salaries.payslip_v3_overtime', compact('salarySheet', 'words', 'allowances', 'allowancesTotal', 'settings', 'comLogoPath'));
        } else {
            $pdf = PDF::loadView('employee_salaries.payslip_v3', compact('salarySheet', 'words', 'allowances', 'allowancesTotal', 'settings', 'comLogoPath'));
        }
