...
class InvoiceController extends AppBaseController
{
  ...
  public function index()
    ...
    public function show(Invoice $invoice)
    {
        ...
        // Use taxable if set, otherwise fall back to quantity * rate
        foreach ($invoice->salesItems as $item) {
            $itemTaxable = (floatval($item->taxable) > 0)
                ? floatval($item->taxable)
                : (floatval($item->quantity) * floatval($item->rate));
