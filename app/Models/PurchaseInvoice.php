 ...
 */
class PurchaseInvoice extends Model implements Taggable
{
    ...
    public static $messages = [
        ...
        // 'itemsArr.*.service_id.required' => 'Service is required',
    ...
    {
        return $this->morphMany(PurchasedItem::class, 'owner');
    }
    ...
    {
        return $this->hasMany(EstimateAddress::class, 'estimate_number', 'estimate_number');
    }
    ...
    {
        return $this->hasMany(PurchaseInvoiceTerm::class, 'estimate_id');
    }
    ...
    {
        return $this->hasMany(PurchaseInvoiceDoc::class);
    }
    ...
    {
        if (!empty($value)) {
            $this->attributes['estimate_date'] = \Carbon\Carbon::createFromFormat('d-m-Y', $value)->format('Y-m-d H:i:s');
        } else {
            $this->attributes['estimate_date'] = null;
        }
    }

      public function returns()
    {
        return $this->belongsTo(PurchaseReturn::class,'estimate_number','purchase_invoice_id');
    }

     public function payments(): MorphMany
    {
        return $this->morphMany(PurchasePayment::class, 'owner');
    }
